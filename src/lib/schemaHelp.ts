...
export const SCHEMA_HELP_SECTIONS: HelpSection[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      { name: "zeeman", type: "number | object", description: "Expand the state into Zeeman sublevels. A number is shorthand for `zeeman.energy_step`." },
      { name: "zeeman.energy_step", type: "number", description: "Optional energy shift applied per `Δm = 1`. Default keeps all sublevels at the same energy." },
      { name: "zeeman.step", type: "number", description: "Backward-compatible alias for `zeeman.energy_step`." },
      { name: "zeeman.values", type: "number[]", description: "Explicit magnetic quantum numbers. Defaults to `-J ... +J`, laid out from left to right." },
      { name: "zeeman.width", type: "number", description: "Width of each Zeeman sublevel as a fraction of the original state width. Default is `0.25`." },
      { name: "zeeman.label_position", type: "above | below", description: "Place the per-sublevel `m_J` labels above or below the Zeeman bars." },
  ...
  },
  {
    ...
    fields: [
      ...
      { name: "arrows", type: "none | single | double", description: "Preferred arrow mode for the transition. Default is `double`." },
