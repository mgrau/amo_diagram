...
function trimPolylineEndpoints(points: [number, number][], startTrim: number, endTrim: number): [number, number][] {
  if (points.length < 2 || (startTrim <= 0 && endTrim <= 0)) {
    return points;
  }
...
function pointAlongPolyline(points: [number, number][], distance: number): [number, number] {
  if (points.length === 0) {
    return [0, 0];
  }
